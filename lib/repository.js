...
Repo.prototype.executeRest = async function(req, res, mode) {
    ...
    if(mode == 'get' || mode == 'audit') {
        try {
            ...
            for (var i = 0; i < this.restParams.length; i++) {
                ...
                rg.getRepo(repoParam, function (name, repo, err) {
                    if (err) {
    ...
    else if(mode == 'create')
    {
        ...
        try
        {
            ...
            var owner = restParams.templateRepoURL.split('/')[3];
            var name = restParams.templateRepoURL.split('/')[4];
        ...
        try
        {
            var templateRepoParam = {owner:owner,name:name};
            var rg = new RG(this.config.ghPAT);
            rg.getRepo(templateRepoParam,async function (name, repo, err) {
                if (err) {
                    that.repos.set(name, "ERROR: " + err.message);
                }
                else
                {
                    try{
                        var newRepo = await that.createRepo(repo, restParams.newRepoOwner, restParams.newRepoName,tokens);
                    }
                    catch(err)
                    {
                        that.respond(501,"Error creating repo",err);
                        return;
                    }

                    that.respond(201,newRepo);
    ...
    else if(mode == 'create')
    {
        try {
            var owner = restParams.templateRepoURL.split('/')[3];
            var repo = restParams.templateRepoURL.split('/')[4];
        }
        catch(err)
        {
            this.respond(501,"Error parsing configuration",err);
        ...
        newRepoData = await this.createRepo(repoData, restParams.newRepoName, restParams.newRepoOwner, restParams.tokens);
    ...
    */
    else if(mode == 'audit')
    {
        ...
        try {
            var repo1owner = restParams.repo1URL.split('/')[3];
            var repo1name = restParams.repo1URL.split('/')[4];
            var repo2owner = restParams.repo2URL.split('/')[3];
            var repo2name = restParams.repo2URL.split('/')[4];
        }
        catch(err)
        {
            this.respond(501,"Error parsing configuration",err);
...
Repo.prototype.createRepo = async function(templateRepo, newRepoOwner,newRepoName, tokens)
{


    var newRepo = {repo:{},tree:{},users:[],teams:[],branches:[],hooks:[],errors:[]};
    ...
    try {
        ...
        newRepo.repo.name = repo.data.name;
        newRepo.repo.description = repo.data.description;
        newRepo.repo.has_issues = repo.data.has_issues;
        newRepo.repo.has_projects = repo.data.has_projects;
        newRepo.repo.has_wiki = repo.data.has_wiki;
        newRepo.repo.has_pages = repo.data.has_pages;
        newRepo.repo.allow_squash_merge = repo.data.allow_squash_merge;
        newRepo.repo.allow_merge_commit = repo.data.allow_merge_commit;
        newRepo.repo.allow_rebase_merge = repo.data.allow_rebase_merge;
        newRepo.repo.organization = {};
        newRepo.repo.organization.login = repo.data.organization.login;

        if(templateRepo.hasOwnProperty('teams'))
        {
            for(var t = 0; t < templateRepo.teams.length;t++)
            {
                var team = await this.github.orgs.addTeamRepo({
                    ...
                    ,owner:newRepoOwner
                    ,repo:newRepoName
                    ,permission:templateRepo.teams[t].permission
                });
            }
        }
        ...
        if(templateRepo.hasOwnProperty('users'))
        {
            for(var u = 0; u < templateRepo.users.length; u++)
            {
                var user = await this.github.repos.addCollaborator({
                     owner:newRepoOwner
                    ,repo:newRepoName
                    ,username:templateRepo.users[u].login
                    ,permission:templateRepo.users[u].permission
                });
            }
        }
        ...
        for(u = 0; u < templateRepo.tree.tree.length; u++) {
            content = await
            this.github.repos.getContent({
                owner: templateRepoOwner
                , repo: templateRepoName
                , path: templateRepo.tree.tree[u].path
                , ref: 'master'
            });

            if (templateRepo.tree.tree[u].path != 'README.md' && (templateRepo.tree.tree[u].type == 'file' || templateRepo.tree.tree[u].type == 'blob')) {
                newContent = await
            ...
            else if (templateRepo.tree.tree[u].path == 'README.md')
            {
        ...
        for(var i = 0; i < templateRepo.branches.length;i++)
        {
            var branch = templateRepo.branches[i];
...
Repo.prototype.getRepo = async function(owner,name)
{
    this.start = new Date();
