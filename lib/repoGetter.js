...
function repoGetter(ghPAT, templateRepoParam,caller) {
    ...
    this.templateRepoParam = templateRepoParam;
    this.caller = caller;
...
try {
    ...
    if(err)
    {
        this.repoData.errors.push(err.message);
    }
...
{
    try {
        var topics = await this.github.repos.getTopics({
            owner: this.templateRepoParam.owner
            , repo: this.templateRepoParam.name
        });
...
repoGetter.prototype.getRepoData = async function()
{
    ...
    try {
        ...
        repo = await this.github.repos.get({owner:this.templateRepoParams.owner,repo:this.templateRepoParams.name});
...
repoGetter.prototype.getBranches = async function()
{
    try {
        ...
        this.github.repos.getBranches({
             owner:this.templateRepoParams.owner
            ,repo: this.templateRepoParams.name
        });
...
repoGetter.prototype.getHooks = async function()
{
    try {
        ...
        console.log("HOOKS");
        retval = await this.github.repos.getHooks({
            owner:this.templateRepoParams.owner
            ,repo: this.templateRepoParams.name
        });
