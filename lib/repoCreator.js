...
repoCreator.prototype.createLabels = async function()
{
    try {
        ...
        for (var l = 0; l < this.templateRepo.labels.length; l++) {
            var label = this.templateRepo.labels[l];
            var newLabel = await this.github.issues.createLabel({
                owner: this.newRepoOwner
                , repo: this.newRepoName
                , name: label.name
                , color: label.color
...
repoCreator.prototype.createTopics = async function()
{
    try {
        ...
        for (var l = 0; l < this.templateRepo.topics.length; l++) {
            var Topic = this.templateRepo.topics[l];
            var newTopic = await this.github.issues.createTopic({
                owner: this.newRepoOwner
                , repo: this.newRepoName
                , name: topic.name
                , color: topic.color
