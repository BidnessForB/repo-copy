...
RepoCreator.prototype.execute = async function() {
    ...
    try
    {
        await this.createRepo(); //can't do anything until the repo is created
    }
    catch(err)
    {
        throw new Error(err);
	...
	await this.createTree(); //can't create the tree after create branches, in case there are protections
    this.createUsers();
    this.createTeams();
    this.createHooks();
    this.createLabels();
...
RepoCreator.prototype.createBranches = async function()
{
	...
	appDebug("Creating BRANCHES for " +  this.newRepoOwner + '/' + this.newRepoName);
	...
	try {
        ...
        {
            appDebug("ERROR creating branches" + err.message);
...
RepoCreator.prototype.createUsers = async function()
{
	...
	appDebug("Creating USERS for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.createTeams = async function()
{
	...
	appDebug("Creating TEAMS for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.createHooks = async function()
{
    ...
    appDebug("Creating HOOKS for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.createLabels = async function()
{
	...
	appDebug("Creating LABELS for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.createTopics = async function()
{
	...
	appDebug("Creating TOPICS for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.createTree = async function ()
{
	...
	appDebug("Creating TREE for " + this.newRepoOwner + '/' + this.newRepoName);
...
RepoCreator.prototype.callback = function(step,value,err)
{
        ...
        appDebug("Finished creating " + step.toUpperCase() + " for " + this.newRepoOwner + "/" + this.newRepoName);
...
RepoCreator.prototype.createRepo = async function()
{
	...
	appDebug("Creating REPO for " + this.newRepoOwner + '/' + this.newRepoName);

    try {
		repo = await this.github.repos.createForOrg({
			org: this.newRepoOwner
			,
			name: this.newRepoName
			,
			description: (this.templateRepo.repo.description ? this.transformContent(this.templateRepo.repo.description, this.tokens, 'string') : 'Copied from ' + this.templateRepo.repo.organization.login + '/' + this.templateRepo.repo.name)
			,
			auto_init: true //create a readme, and by virtue of that, a master branch
		});

		masterRef = await this.github.gitdata.getReference({
            owner:this.newRepoOwner
            ,repo: this.newRepoName
            ,ref:'heads/master'
        });
		this.masterSHA = masterRef.data.object.sha;
	}
    catch(err)
        {
